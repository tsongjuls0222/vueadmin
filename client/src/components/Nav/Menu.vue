  ...
  <div class="menu-wrapper">
    <div class="main-menu-wrapper">
      <ul class="menu-list level">
        ...
        >
          <div>
            {{ item.title }}
            <span class="navmenucount" v-if="item.id == 'game-list-ended'">({{ memberlist_count }})</span>
          </div>
        </li>
        <li class="notorius-badge" >
          <i class="mdi mdi-account" @click="closeDropdownBadge"></i>
          <div class="custom-badge" @click="closeDropdownBadge">{{badgeCount}}</div>
          <div :class="`custom-badge-dropdown ${badgeClass}`">
              <div class="list-header">
                  <h5>Notorious User Betting</h5><i class="mdi mdi-close-circle-outline" @click="closeDropdownBadge"></i>
              </div>
              <div class="list-content">
                <b-table
                  :data="notoriusData"
                >
                  <b-table-column :centered="true" field="ia_name" label="총판" v-slot="props">
                    {{ props.row.ia_name }}
                  </b-table-column>
                  <b-table-column :centered="true" field="real_code" label="코드" v-slot="props">
                    {{ props.row.real_code }}
                  </b-table-column>
                  <b-table-column :centered="true" field="username" label="아이디" v-slot="props">
                    {{ props.row.username }}
                  </b-table-column>
                  <b-table-column :centered="true" field="iu_nickname" label="닉네임" v-slot="props">
                    {{ props.row.iu_nickname }}
                  </b-table-column>
                  <b-table-column :centered="true" field="iu_name" label="이름" v-slot="props">
                    {{ props.row.iu_name }}
                  </b-table-column>
                  <b-table-column :centered="true" field="bet_amount" label="베팅금액" v-slot="props">
                    {{ props.row.bet_amount }}
                  </b-table-column>
                  <b-table-column :centered="true" field="estimated_win" label="당첨금액" v-slot="props">
                    {{ props.row.estimated_win }}
                  </b-table-column>
                  <b-table-column :centered="true" field="is_realtime" label="유형" v-slot="props">
        ...
        <li class="toggle">
          <div class="theme-toggle-wrapper">
              <div class="theme-toggle-content">
                  <div :class="`theme-toggle ${darkModeClass}`" @click="changeMode()">
                      <div class="moon-sun">
                          <div class="moon" >
                              <svg xmlns="http://www.w3.org/2000/svg" width="10.518" height="11.768" viewBox="0 0 10.518 11.768">
                                  <path id="icon-moon" d="M7.8,11.768A5.873,5.873,0,0,0,12.368,9.59a.276.276,0,0,0-.266-.445A4.61,4.61,0,0,1,8.959.609.276.276,0,0,0,8.873.1,5.885,5.885,0,1,0,7.8,11.768Z" transform="translate(-1.913 0)" fill="#ffee01"></path>
                              </svg>
                          </div>
                          <div class="sun">
                              <svg xmlns="http://www.w3.org/2000/svg" width="14.949" height="14.949" viewBox="0 0 14.949 14.949">
                                  <path id="icon-sun" d="M7.475,4.672a2.8,2.8,0,1,0,2.8,2.8A2.807,2.807,0,0,0,7.475,4.672Zm7.194,2.35L11.9,5.641l.978-2.931a.5.5,0,0,0-.639-.639l-2.931.978L7.927.28a.506.506,0,0,0-.905,0L5.641,3.045,2.707,2.067a.5.5,0,0,0-.639.639l.978,2.931L.28,7.022a.506.506,0,0,0,0,.905L3.045,9.308l-.978,2.934a.5.5,0,0,0,.639.639L5.638,11.9l1.381,2.765a.506.506,0,0,0,.905,0L9.305,11.9l2.931.978a.5.5,0,0,0,.639-.639L11.9,9.311,14.663,7.93a.506.506,0,0,0,.006-.908Zm-4.552,3.1a3.737,3.737,0,1,1,0-5.285A3.741,3.741,0,0,1,10.117,10.117Z" transform="translate(0 0)" fill="#ffee01"></path>
                              </svg>
                          </div>
                      </div>
                      <div class="circle">
                          <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17">
                              <circle id="circle" data-name="Ellipse 125" cx="8.5" cy="8.5" r="8.5" fill="#fff"></circle>
                          </svg>
                      </div>
                  </div>
              </div>
    ...
    <div class="sub-menu-wrapper">
      <ul
        ...
        :key="key"
      >
        <li :class="setClass3(subItem.idto)" v-for="(subItem, key) in item.subItems" :key="key">
          <a class="" :href="subItem.to" exact-active-class="is-active">
            {{ subItem.subTitle }}
            <span class="navmenucount" v-if="subItem.id == 'member-list-sub1'">({{ memberlist_count }})</span>
            <span class="navmenucount" v-if="subItem.id == 'member-list-sub2'">({{ memberlisttest_count }})</span>
            <span class="navmenucount" v-if="subItem.id == 'member-list-sub3'">({{ memberlistchongpan_count }})</span>
          </a>
        </li>
      </ul>
    </div>
